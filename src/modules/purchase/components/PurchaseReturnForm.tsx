...
function ImeiBadge({ imei, status }: { imei: string; status: string }) {
  ...
  const c = colors[status] ?? colors.in_stock;
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'JetBrains Mono, monospace', fontSize: 11.5, padding: '3px 7px 3px 6px', borderRadius: 3, background: c.bg, border: `1px solid ${c.border}`, color: C.ink2, letterSpacing: '0.02em' }}>
      ...
      <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: c.label }}>
        {status === 'in_stock' ? 'In stock' : status === 'sold' ? 'Sold' : 'Returned'}
      </span>
    </span>
  );
}

function PayTag({ type }: { type: string }) {
  const labels: Record<string, string> = { cash: 'Cash', credit: 'Credit', partial: 'Partial' };
  return <span style={{ fontSize: 12.5, color: C.ink2, fontWeight: 500 }}>{labels[type] ?? type}</span>;
}
...
function OriginalInvoiceCard({ invoice }: { invoice: PurchaseInvoiceDetail }) {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 14px', borderBottom: `1px dashed ${C.line2}`, fontSize: 11, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
        ...
        <span style={{ marginLeft: 'auto', fontSize: 11, color: C.muted2, textTransform: 'none', letterSpacing: 0, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          ...
          <Dot />
          <span>Created {invoice.created_at?.slice(0, 10) ?? invoice.invoice_date}</span>
        </span>
      </div>

      {/* Meta grid — Invoice no, Supplier, Date, Payment, Remarks */}
      <div style={{ display: 'grid', gridTemplateColumns: '130px 1.4fr 130px 130px 1fr', borderBottom: `1px dashed ${C.line2}` }}>
        {[
          ...
          { k: 'Supplier', v: invoice.supplier_name, sub: null },
          { k: 'Date', v: <span style={{ fontFamily: 'JetBrains Mono, monospace' }}>{invoice.invoice_date}</span>, sub: null },
          { k: 'Payment', v: <PayTag type={invoice.payment_type} />, sub: invoice.payment_type === 'partial' && invoice.cash_amount != null ? `Cash ${fmtNum(invoice.cash_amount)} · Credit ${fmtNum(invoice.credit_amount ?? 0)}` : null },
          { k: 'Remarks', v: <span style={{ fontWeight: 400, color: C.ink2 }}>{invoice.remarks || '—'}</span>, sub: null },
        ].map((cell, i) => (
      ...
      {/* Line table — Item, Qty, Rate (cost), Line total */}
      <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: 13, tableLayout: 'fixed' }}>
        ...
        <thead>
          <tr>
            {['Item', 'Qty', 'Rate (cost)', 'Line total'].map((h, i) => (
          ...
          {invoice.lines.map((line, i) => {
            ...
            return (
              <tr key={line.id}>
                ...
                <td style={{ padding: 12, borderBottom: isLast ? 'none' : `1px dashed ${C.line2}`, textAlign: 'right', fontFamily: 'JetBrains Mono, monospace', verticalAlign: 'top' }}>{fmt(line.rate)}</td>
...
export function PurchaseReturnForm({ initialInvoiceId, onSaved, onCancel }: Props) {
  ...
  const { selLines, totalUnits, totalAmount } = useMemo(() => {
    if (!invoice) return { selLines: 0, totalUnits: 0, totalAmount: 0 };
    let selLines = 0, totalUnits = 0, totalAmount = 0;
    ...
    return { selLines, totalUnits, totalAmount };
  }, [invoice, lineStates]);
  ...
  const handleSave = useCallback(async () => {
    ...
    for (const line of invoice.lines) {
      ...
      if (qty === 0) continue;
      lines.push({ purchase_invoice_line_id: line.id, quantity_returned: qty, imeis: mobile ? Array.from(s.selectedImeis) : [] });
    }
    if (lines.length === 0) return;
    const input: SavePurchaseReturnInput = {
    ...
    setError(null);
    try {
      await savePurchaseReturn(input);
      onSaved();
    } catch (e) {
      setError(String(e));
      setSaving(false);
  ...
  const filteredInvoices = useMemo(() => {
    ...
    return invoiceList.filter(inv =>
      inv.invoice_no.toLowerCase().includes(q) || inv.supplier_name.toLowerCase().includes(q)
    );
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24 }}>
          <div>
            ...
            <div style={{ marginTop: 4, fontSize: 12.5, color: C.muted }}>
              Tick the lines and units you are sending back to the supplier. The original invoice above is read-only.
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontFamily: 'JetBrains Mono, monospace', fontSize: 11.5, color: C.muted }}>
            <span>Auto-ref <b style={{ color: C.ink2, fontWeight: 500 }}>PR-draft</b></span>
          </div>
        </div>
          ...
          {invoice ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
              ...
              <span style={{ fontSize: 12.5, color: C.muted, display: 'flex', alignItems: 'center', gap: 8, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                <b style={{ color: C.ink2, fontWeight: 500 }}>{invoice.supplier_name}</b>
                <Dot /><span>{invoice.invoice_date}</span>
              ...
              <div style={{ maxHeight: 280, overflowY: 'auto' }}>
                ...
                {!loadingList && filteredInvoices.map(inv => (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 13, color: C.ink2, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{inv.supplier_name}</span>
                    <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: C.muted, flexShrink: 0 }}>{inv.invoice_date}</span>
          ...
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, color: C.muted, margin: '2px 4px' }}>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 11, fontWeight: 600, color: C.accent, textTransform: 'uppercase', letterSpacing: '0.1em', padding: '3px 10px', background: 'rgba(31,58,138,0.06)', border: '1px solid rgba(31,58,138,0.22)', borderRadius: 999 }}>
                <svg viewBox="0 0 16 16" width="11" height="11" style={{ stroke: C.accent, fill: 'none', strokeWidth: 1.6, strokeLinecap: 'round', strokeLinejoin: 'round' }}><path d="M8 3v9" /><path d="M5 9l3 3 3-3" /></svg>
            ...
            <div style={{ background: C.paper, border: `1px solid ${C.line}`, borderRadius: 6 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: C.subtle, borderBottom: `1px solid ${C.line}` }}>
                ...
                <div style={{ color: C.muted, fontSize: 12 }}>
                  Only units still <b style={{ color: C.ink2, fontWeight: 500 }}>in stock</b> can be sent back. Sold / already-returned units are locked.
                </div>
                <div style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 12, fontSize: 11, color: C.muted }}>
                  {[
                    { pip: `rgba(31,58,138,0.22)`, border: `rgba(31,58,138,0.5)`, label: 'In stock · returnable' },
                    { pip: C.accent, border: C.accent, label: 'Selected' },
                    { pip: C.subtle, border: C.line2, label: 'Sold / returned · locked' },
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '36px minmax(220px, 1fr) 80px 130px 140px 160px', background: C.subtle, cursor: 'default' }}>
                ...
                <div style={{ padding: '10px 12px', textAlign: 'right' }}><ColLabel>Rate</ColLabel></div>
                    ...
                    <div key={line.id} style={{
                      ...
                      <div style={{ padding: '14px 12px' }}>
                        ...
                        <div style={{ color: C.muted, fontSize: 11.5, marginTop: 2 }}>
                          {mobile
                            ? `${retImeis.length} of ${line.quantity} unit${line.quantity !== 1 ? 's' : ''} still in stock · pick IMEIs to return`
                      ...
                      <div style={{ padding: '14px 12px', textAlign: 'right', fontFamily: 'JetBrains Mono, monospace', fontSize: 12.5, color: C.muted }}>
                        <b style={{ color: C.ink2, fontWeight: 500 }}>{fmtNum(line.rate)}</b>
                        <br />
                      ...
                      <div style={{ padding: '14px 12px', textAlign: 'right' }}>
                        {s.checked ? (
                          <>
                            ...
                            <div style={{ fontSize: 11, color: C.muted }}>of {mobile ? retImeis.length : line.quantity} in stock</div>
                          </>
                        ) : (
                          <>
                      ...
                      {s.checked && (
                        <div style={{ gridColumn: '2 / -1', padding: '0 12px 12px', marginTop: -2 }}>
                          {mobile ? (
                            /* IMEI picker — in_stock = returnable (blue), sold/returned = locked */
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, padding: 10, background: C.paper, border: `1px dashed ${C.line2}`, borderRadius: 4 }}>
                              <div style={{ width: '100%', fontSize: 11, color: C.muted, marginBottom: 4 }}>
                                {retImeis.length} in-stock unit{retImeis.length !== 1 ? 's' : ''} returnable.{' '}
                                <b style={{ color: C.ink2 }}>{s.selectedImeis.size} selected</b>.
                                {(() => {
                                  const sold = line.imeis.filter(i => i.status === 'sold').length;
                                  const ret = line.imeis.filter(i => i.status === 'returned').length;
                                  const parts = [];
                                  if (sold) parts.push(`${sold} sold`);
                                  if (ret) parts.push(`${ret} already returned`);
                                  return parts.length > 0 ? ` ${parts.join(', ')} — locked.` : '';
                                })()}
                              </div>
                              {line.imeis.map(imei => {
                                const isReturnable = imei.status === 'in_stock';
                                ...
                                return (
                                  <div
                                    ...
                                    style={{
                                      ...
                                      ...(isReturnable && isSel
                                        ...
                                        : isReturnable
                                        ? { background: C.paper, border: '1px solid rgba(31,58,138,0.32)', color: C.ink2 }
                                  ...
                                  >
                                    ...
                                    {/* pip */}
                                    <span style={{ width: 6, height: 6, borderRadius: '50%', flexShrink: 0, background: isReturnable ? (isSel ? 'rgba(255,255,255,0.85)' : C.info) : imei.status === 'sold' ? C.ok : C.warn, opacity: isReturnable ? 1 : 0.6 }} />
                                    {imei.imei}
                                    {!isReturnable && (
                                      <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: imei.status === 'sold' ? C.ok : C.warn, opacity: 0.8 }}>
                                        {imei.status === 'sold' ? 'Sold' : 'Returned'}
                                      </span>
                                    )}
                                  </div>
                                );
                            ...
                            <div style={{ padding: '10px 12px', background: C.paper, border: `1px dashed ${C.line2}`, borderRadius: 4, display: 'flex', alignItems: 'center', gap: 14 }}>
                              ...
                              <span style={{ fontSize: 11.5, color: C.muted2, fontFamily: 'JetBrains Mono, monospace' }}>max {line.quantity} · pulled from on-hand stock</span>
              ...
              <div style={{ padding: 16, borderRight: `1px solid ${C.line}`, display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    {(['today', 'yesterday', 'invoice'] as const).map(preset => (
                      <span
                        ...
                        onClick={() => {
                          ...
                          else setReturnDate(invoice.invoice_date);
                        }}
                        style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <textarea
                    ...
                    placeholder="e.g. Units found defective on QC — returning to supplier under warranty."
                ...
                {totalAmount > 0 && (
                  <div style={{ border: `1px dashed ${C.line2}`, background: C.subtle, borderRadius: 4, padding: '10px 12px', fontFamily: 'JetBrains Mono, monospace', fontSize: 11.5, color: C.muted, display: 'flex', flexDirection: 'column', gap: 4 }}>
                    ...
                    {[
                      { side: 'DR', label: `A/P · ${invoice.supplier_name}`, amt: totalAmount },
                      { side: 'CR', label: 'Inventory', amt: totalAmount },
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 4, color: C.muted2 }}>
                      <span style={{ width: 30, textAlign: 'center' }}>·</span>
                      <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.06em', color: C.muted }}>Stock side-effects</span>
                    </div>
                    {invoice.lines.filter(l => {
                      const s = lineStates[l.id];
                      return s?.checked && lineUnits(l, s) > 0;
                    }).map(l => {
                      const s = lineStates[l.id];
                      const mobile = isMobileLine(l);
                      const units = lineUnits(l, s);
                      return (
                        <div key={l.id} style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                          <span style={{ width: 30, color: C.muted, textAlign: 'center' }}>→</span>
                          <span style={{ flex: 1, color: C.ink2 }}>
                            {mobile
                              ? <>{units} IMEI{units !== 1 ? 's' : ''} → status <b style={{ color: C.warn }}>returned</b></>
                              : <>{l.item_name} stock −{units} pc{units !== 1 ? 's' : ''}</>
                            }
                          </span>
