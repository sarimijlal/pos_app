...
function getPresetRange(p: DatePreset): { from: string; to: string } | null {
  const today = todayStr();
  if (p === 'today') return { from: today, to: today };
  if (p === 'yesterday') { const y = offsetDate(-1); return { from: y, to: y }; }
  if (p === '7d') return { from: offsetDate(-6), to: today };
  if (p === '30d') return { from: offsetDate(-29), to: today };
  if (p === 'month') return { from: startOfMonth(), to: today };
  return null;
}

function presetLabel(p: DatePreset, from: string, to: string) {
...
function StatusBadge({ status }: { status: 'active' | 'returned' }) {
  ...
  return (
    <span style={{
      ...
      height: 22, padding: '0 9px 0 7px', borderRadius: 999,
...
function AppliedTag({ label, onRemove }: { label: string; onRemove: () => void }) {
  return (
    <span style={{
      ...
      <span
        onClick={onRemove}
        style={{ width: 14, height: 14, borderRadius: 2, display: 'inline-grid', placeItems: 'center', cursor: 'pointer' }}
      >
...
export function PurchaseListScreen({ onNew, onViewDetail, onReturn }: Props) {
  ...
  useEffect(() => {
    function onDocClick(e: MouseEvent) {
      ...
      if (supplierFieldRef.current && !supplierFieldRef.current.contains(t)) setSupplierPop(false);
    }
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if ((e.key === 'ArrowDown' || e.key === 'ArrowUp') && !inInput) {
        ...
        tbodyRef.current?.querySelector(`[data-id="${paginated[next].id}"]`)?.scrollIntoView({ block: 'nearest' });
        return;
      }
      if (e.key === 'Enter' && !inInput && selectedId) { onViewDetail(selectedId); }
    }
  ...
  function setPreset(p: DatePreset) {
    ...
    setFilters(f => ({ ...f, datePreset: p, dateFrom: range?.from ?? '', dateTo: range?.to ?? '' }));
    setCustomDateFrom(range?.from ?? '');
    setCustomDateTo(range?.to ?? '');
    setPage(1);
  }

  function applyFilters(): PurchaseInvoiceRow[] {
    const q = filters.search.trim().toLowerCase();
    return rows.filter(r => {
      if (filters.supplierId !== null && r.supplier_id !== filters.supplierId) return false;
      if (filters.payment !== 'all' && r.payment_type !== filters.payment) return false;
      if (filters.dateFrom && r.invoice_date < filters.dateFrom) return false;
      if (filters.dateTo && r.invoice_date > filters.dateTo) return false;
      if (q) {
      ...
      let cmp = 0;
      if (sortCol === 'date') cmp = a.invoice_date.localeCompare(b.invoice_date) || a.invoice_no.localeCompare(b.invoice_no);
  ...
  function paginate(filtered: PurchaseInvoiceRow[]) {
  ...
  function clearAllFilters() {
    setFilters({ supplierId: null, payment: 'all', search: '', datePreset: 'all', dateFrom: '', dateTo: '' });
    setCustomDateFrom(''); setCustomDateTo('');
    if (searchRef.current) searchRef.current.value = '';
    setPage(1); closeAllPops();
  }
  ...
  const sortIcon = (col: SortCol) => sortCol === col ? (sortDir === 'desc' ? '↓' : '↑') : '↕';
  ...
  const popRowStyle = (on: boolean): React.CSSProperties => ({
    padding: '7px 10px', borderRadius: 4, cursor: 'pointer',
    display: 'flex', alignItems: 'center', gap: 10, fontSize: 13,
    background: on ? C.infoBg : 'transparent',
    color: on ? C.info : C.ink2,
  });
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24 }}>
        <div>
          ...
          <div style={{ marginTop: 4, fontSize: 12.5, color: C.muted }}>
            All purchase invoices. Click any row to view detail, or use the actions on the right.
          </div>
        </div>
      ...
      <div style={{ background: C.paper, border: `1px solid ${C.line}`, borderRadius: 6 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(260px, 1.2fr) 1.1fr 1.1fr 1.3fr', gap: 0 }}>
          ...
          <div ref={dateFieldRef} style={{ display: 'flex', flexDirection: 'column', gap: 5, padding: '12px 14px', borderRight: `1px solid ${C.line}`, position: 'relative' }}>
            ...
            <button
              ...
              onClick={() => { setDatePop(v => !v); setSupplierPop(false); }}
              type="button"
            >
              <span style={{ color: C.muted, fontSize: 11.5, flexShrink: 0 }}>📅</span>
              <span style={{ flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: (filters.datePreset === 'all' && !filters.dateFrom) ? C.muted2 : C.ink }}>
                {filters.datePreset === 'all' && !filters.dateFrom ? 'Any date' : presetLabel(filters.datePreset, filters.dateFrom, filters.dateTo)}
              </span>
            ...
            {datePop && (
              <div style={popStyle}>
                ...
                <div style={{ borderTop: `1px solid ${C.line}`, padding: '8px 10px', display: 'flex', background: C.subtle, fontSize: 11.5, color: C.muted }}>
                  <span
                    style={{ marginLeft: 'auto', color: C.accent, cursor: 'pointer', fontWeight: 500 }}
                    onClick={() => {
          ...
          {/* SUPPLIER */}
          <div ref={supplierFieldRef} style={{ display: 'flex', flexDirection: 'column', gap: 5, padding: '12px 14px', borderRight: `1px solid ${C.line}`, position: 'relative' }}>
            <label style={{ fontSize: 10.5, fontWeight: 600, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Supplier</label>
            <button
              style={filterCtrlStyle(filters.supplierId !== null)}
              onClick={() => { setSupplierPop(v => !v); setDatePop(false); if (!supplierPop) setTimeout(() => supplierSearchRef.current?.focus(), 30); }}
              type="button"
            >
              <span style={{ flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: selectedSupplier ? C.ink : C.muted2 }}>
              ...
              <div style={popStyle}>
                <div style={{ borderBottom: `1px solid ${C.line}`, padding: '8px 10px', display: 'flex', alignItems: 'center', gap: 8, background: C.subtle }}>
                  ...
                  <input
                    ...
                    style={{ border: 0, outline: 0, background: 'transparent', fontFamily: 'inherit', fontSize: 13, color: C.ink, flex: 1 }}
                  />
                </div>
                <div style={{ maxHeight: 240, overflowY: 'auto', padding: 4 }}>
                  <div onClick={() => { setFilters(f => ({ ...f, supplierId: null })); setSupplierPop(false); setSupplierSearch(''); setPage(1); }} style={popRowStyle(filters.supplierId === null)}>
                    <div style={{ flex: 1 }}>All suppliers</div>
                    {filters.supplierId === null && <span style={{ color: C.info }}>✓</span>}
                  </div>
                  {filteredSuppliers.map(s => (
                    <div key={s.id} onClick={() => { setFilters(f => ({ ...f, supplierId: s.id })); setSupplierPop(false); setSupplierSearch(''); setPage(1); }} style={popRowStyle(filters.supplierId === s.id)}>
                      <div style={{ flex: 1, minWidth: 0 }}>{s.name}</div>
                      {filters.supplierId === s.id && <span style={{ color: C.info }}>✓</span>}
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5, padding: '12px 14px', borderRight: `1px solid ${C.line}` }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'stretch', border: `1px solid ${C.line2}`, borderRadius: 4, background: C.subtle, padding: 2, gap: 2, height: 32, width: '100%', overflow: 'hidden' }}>
              {(['all', 'cash', 'credit', 'partial'] as PayType[]).map(m => (
                <button
                  ...
                  style={{
                    appearance: 'none',
                    border: filters.payment === m ? `1px solid ${C.line2}` : 0,
                    background: filters.payment === m ? C.paper : 'transparent',
                    fontFamily: 'inherit', fontSize: 12, fontWeight: 500,
                    color: filters.payment === m ? C.ink : C.muted,
                    padding: '0 8px', borderRadius: 3, cursor: 'pointer', flex: 1, whiteSpace: 'nowrap', minWidth: 0,
            ...
            <div style={{ fontSize: 11, color: C.muted2 }}>Cash · Credit · Partial</div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5, padding: '12px 14px' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input
                ref={searchRef}
                type="text"
                placeholder="Invoice no., supplier or IMEI…"
                onChange={e => { setFilters(f => ({ ...f, search: e.target.value })); setPage(1); }}
                style={{ height: 32, padding: '0 32px 0 32px', background: C.paper, border: `1px solid ${C.line2}`, borderRadius: 4, fontFamily: 'inherit', fontSize: 13, color: C.ink, width: '100%', outline: 'none' }}
              />
            ...
            <div style={{ fontSize: 11, color: C.muted2 }}>Matches invoice no., supplier or IMEI</div>
          </div>
        </div>
        ...
        <div style={{
          ...
          {filters.supplierId !== null && selectedSupplier && (
            <AppliedTag label={`Supplier: ${selectedSupplier.name}`} onRemove={() => { setFilters(f => ({ ...f, supplierId: null })); setPage(1); }} />
          )}
          {filters.payment !== 'all' && (
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          {loadState === 'loading' ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: 13, tableLayout: 'fixed' }}>
              ...
              <thead>
                <tr>
                  <th style={thStyle(true, sortCol === 'no')} onClick={() => toggleSort('no')}>
                    Invoice no. <span style={{ display: 'inline-block', marginLeft: 4, opacity: sortCol === 'no' ? 1 : 0.45 }}>{sortIcon('no')}</span>
                  </th>
                  <th style={thStyle(true, sortCol === 'date')} onClick={() => toggleSort('date')}>
              ...
              <tbody ref={tbodyRef}>
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={7} style={{ padding: '40px 12px', textAlign: 'center', color: C.muted, fontSize: 13, borderBottom: 'none' }}>
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Date */}
                      <td style={tdStyle}>
                        <div style={{ fontFamily: "'JetBrains Mono', monospace", color: C.ink2, fontSize: 12.5 }}>{inv.invoice_date}</div>
                        <div style={{ color: C.muted, fontSize: 11.5, marginTop: 1 }}>
                          {inv.created_at ? new Date(inv.created_at).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }) : ''}
                        </div>
                      </td>
                      {/* Supplier */}
                      <td style={tdStyle}>
                        <div style={{ color: C.ink }}>{inv.supplier_name}</div>
                        {inv.remarks && (
                          <div style={{ color: C.muted, fontSize: 11.5, marginTop: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{inv.remarks}</div>
                        )}
                      </td>
                      {/* Total */}
                      <td style={{
                        ...
                        color: C.ink, fontWeight: 600, fontSize: 13.5,
                      ...
                      <td style={{ ...tdStyle, textAlign: 'right' }}>
                        <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end', alignItems: 'center' }}>
                          <button
                            ...
                            style={{ height: 26, padding: '0 9px', border: `1px solid ${C.line2}`, background: C.paper, color: C.ink2, fontFamily: 'inherit', fontSize: 12, fontWeight: 500, borderRadius: 4, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 5 }}
                          >
                          ...
                          <button
                            ...
                            style={{
                              ...
                              background: C.paper, color: returned ? C.muted2 : C.warn,
                              fontFamily: 'inherit', fontSize: 12, fontWeight: 500,
                          ...
                          <button style={{ width: 26, height: 26, border: '1px solid transparent', background: 'transparent', color: C.muted, borderRadius: 4, cursor: 'pointer', display: 'grid', placeItems: 'center' }}>
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 4, fontFamily: "'JetBrains Mono', monospace", fontSize: 11.5,
                  display: 'inline-grid', placeItems: 'center',
                }}
              >
                {btn.label}
