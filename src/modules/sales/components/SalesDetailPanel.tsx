...
import { useReturnSalesInvoice } from '../hooks/useReturnSalesInvoice';
import type { SalesInvoiceDetail, SalesReturnLineInput } from '../types';

export function SalesDetailPanel({
  onClose,
  onReturned,
}: {
  onClose: () => void;
  ...
  const { selectedInvoiceId, saving, error } = useSalesStore();
  const { returnInvoice } = useReturnSalesInvoice();
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setFetchError(null);
    getSalesInvoiceById(selectedInvoiceId)
      .then((d) => {
        setDetail(d);
        setLoading(false);
      })
  ...
  async function reload() {
    if (!selectedInvoiceId) return;
    const d = await getSalesInvoiceById(selectedInvoiceId).catch(() => null);
    setDetail(d);
  }
  ...
  async function handleSubmit() {
    ...
    const lines: SalesReturnLineInput[] = detail.lines
      .filter((l) => selectedLines.has(l.id))
      .map((l) => {
        const isMobile = l.imeis.length > 0;
        return {
          sales_invoice_line_id: l.id,
    ...
    if (id !== null) {
      await reload();
      const refreshed = await getSalesInvoiceById(detail.id).catch(() => null);
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {detail.customer_name} · {detail.date} · {detail.payment_mode}
            {detail.salesperson_name && ` · ${detail.salesperson_name}`}
          </p>
        </div>
          ...
          <tbody>
            {detail.lines.map((line) => (
              <tr key={line.id} className="border-b last:border-0">
                ...
                <td className="px-3 py-2 text-right font-mono">{line.sale_price.toLocaleString()}</td>
                <td className="px-3 py-2 text-right font-mono text-muted-foreground">
                  {line.cost_price.toLocaleString()}
                </td>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {detail.lines.map((line) => {
                ...
                const soldImeis = line.imeis.filter((d) => d.status === 'sold');
                  ...
                  <div key={line.id} className="rounded border p-3 space-y-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        disabled={isMobile && soldImeis.length === 0}
                      />
                      <span className="font-medium text-sm">{line.item_name}</span>
                      <span className="text-xs text-muted-foreground">
                        {isMobile ? `${soldImeis.length} returnable IMEI(s)` : `qty ${line.quantity}`}
                      </span>
                    </label>
